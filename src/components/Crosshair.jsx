...
const Crosshair = ({ color = 'white', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
        gsap.to([lineHorizontalRef.current, lineVerticalRef.current], { opacity: 1, duration: 0.5, ease: 'power3.out' });
    };
    const handleMouseLeave = () => {
        gsap.to([lineHorizontalRef.current, lineVerticalRef.current], { opacity: 0, duration: 0.5, ease: 'power3.out' });
    };
    ...
    const links = containerRef?.current
      ? containerRef.current.querySelectorAll('a, .btn')
